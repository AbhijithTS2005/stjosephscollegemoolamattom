...
<div class="form-container">
    <div class="form-header">
        <h1>Edit Event</h1>
        <a href="{{ route('events.index') }}" class="form-back-link">
        ...
        <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-row">
                ...
                @if(auth()->user()->is_admin)
                    <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                        <label>Event Status (Optional)</label>
                        <select name="status">
                            <option value="approved" {{ old('status', $event->status) == 'approved' ? 'selected' : '' }}>Approved (Published)</option>
                        ...
                        @error('status')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                        <span class="form-helper">Defaults to "Approved" - published immediately</span>
                    </div>
                @endif
            </div>
            ...
            @if(auth()->user()->is_admin)
                <div class="form-group {{ $errors->has('department') ? 'has-error' : '' }}">
                    ...
                    <input 
                        ...
                        placeholder="Leave empty for college-wide events or enter department name"
                    >
                    @error('department')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                    <span class="form-helper">Leave blank for events visible to all, or specify a department (e.g., Physics, Chemistry)</span>
                </div>
            @else
            ...
            <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                ...
                @error('image')
                    <span class="form-error">{{ $message }}</span>
                @enderror
                <span class="form-helper">Supported formats: JPEG, PNG, JPG, GIF, SVG (Max 5MB). Leave empty to keep current image.</span>
                @if ($event->image)
                    <div class="field-hint" style="margin-top: 15px;">
                        <strong>Current Image:</strong><br>
                        <img src="{{ $event->image }}" alt="Event image" style="max-width: 200px; margin-top: 10px; border-radius: 4px;">
                    </div>
                @endif
